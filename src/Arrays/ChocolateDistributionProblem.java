...
public class ChocolateDistributionProblem {
    public static void main(String[] args) {
        ...
        int students = 3;
        System.out.println(findMinimum(a, a.length, students));
    }
    private static int findMinimum(int[] a, int n, int students){
        Arrays.sort(a);
        int minDiff = Integer.MAX_VALUE;
        for(int i = 0;i<n - students + 1;i++){
            if(Math.abs(a[i] - a[i + students - 1]) < minDiff){
                minDiff = Math.abs(a[i] - a[i + students - 1]);
            }
        }
        return minDiff;
    }
